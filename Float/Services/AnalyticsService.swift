...
public enum FloatEvent {
    ...
    var properties: [String: Any] {
        ...
        case let .signInFailed(method, reason):            return ["method": method, "reason": reason]
        case .signUpCompleted:                                 return [:]
        case .signedOut:                                       return [:]
        case let .dealsViewed(count, lat, lon):        return ["count": count, "latitude": lat, "longitude": lon]
        case let .dealTapped(id, venue, cat):          return ["deal_id": id, "venue_id": venue, "category": cat]
        case let .dealRedeemed(id, venue):                 return ["deal_id": id, "venue_id": venue]
        ...
        case .searchQueried(let query):                            return ["query": query]
        case let .searchResultTapped(id, rank):            return ["deal_id": id, "rank": rank]
        ...
        case let .errorOccurred(domain, code, screen): return ["domain": domain, "code": code, "screen": screen]
        }
    }
}
